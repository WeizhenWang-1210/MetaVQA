...
from metadrive.component.sensors.depth_lidar import DepthLidar
from metadrive.component.sensors.depth_camera import DepthCamera
from metadrive.envs.metadrive_env import MetaDriveEnv

blackbox_test_configs = dict(
    # standard=dict(stack_size=3, width=256, height=128, rgb_clip=True),
    small=dict(stack_size=1, width=64, height=64, rgb_clip=False),
)
    ...
    env = MetaDriveEnv(
        {
            ...
            "sensors": {
                "camera": (DepthLidar, config["width"], config["height"])
            },
