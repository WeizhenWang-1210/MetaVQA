...
def record_frame(env: ScenarioDiverseEnv, record=False):
    ...
    positions = [
        (0., 0.0, 1.5)]  #[(0., 0.0, 1.5), (0., 0., 1.5), (0., 0., 1.5), (0., 0, 1.5), (0., 0., 1.5),(0., 0., 1.5)]
...
def closed_loop(env: ScenarioDiverseEnv, seeds, record=True):
    navigation = "FORWARD"
    prompt = f"You are the driver of a car. You are now driving on the street, and your current navigation command is \"{navigation}\". Please pick one of the following action in order to proceed safely:(A) TURN LEFT; (B) TURN RIGHT; (C) BRAKE; (D) NONE. For example, if you think \"TURN LEFT\" action should be chosen, answer \"A\"."
...
def main():
    ...
    import json
    json.dump(summary, open("/home/weizhen", "w"), indent=2)
