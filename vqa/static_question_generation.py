...
from vqa.static_question_generator import GRAMMAR, QuerySpecifier, NAMED_MAPPING
        ...
        for idx in range(attempts):
            ...
            for question, answer in result:
                if question_type == "counting":
                    ...
                    count0, count1 = len(q.parameters["<o1>"]["answer"]), len(q.parameters["<o2>"]["answer"])
                    ...
                    if len(answer) > 1:
                        ...
                        if question_type == "color_identification":
                            ...
                            explanation = "This is the type of thing that all {} {} belong to.".format(len(ids), referral_string)
                    else:
                        if question_type == "color_identification":
                    ...
                    if question_type == "color_identification_unique":
                        ...
                        explanation = "The type of that {} thing is {}.".format \
                            (q.graph.get_node(obj_id).color.lower(),  NAMED_MAPPING[q.graph.get_node(obj_id).type]["singular"])
                    record[id_start + counts + type_count] = dict(
                        question=question,
                        answer=answer,
                        explanation=explanation,
